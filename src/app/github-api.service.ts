...
import { map, multicast,  refCount,    scan, startWith,  switchMap, switchMapTo, tap } from 'rxjs/operators';
...
export class GithubApiService {
  ...
  constructor() {
    ...
    const Ldeleted: Observable<DataUser[]> = this.refreshSubj.pipe(
      switchMap( () => this.deletedSubj.pipe(
        scan( (L, u) => [...L, u], [] ),
        startWith([])
      ) )
    );

    this.usersObs = of([]); // XXX à modifier...
    
    this.refresh();
  }

  async getUsers(): Promise<DataUser[]> {
    const R = await fetch(`https://api.github.com/users?since=${1000*Math.random()}`);
    if (R.status >= 200 && R.status < 300) {
      const L: DataUser[] = await R.json(); // any -> DataUser[]
      return L;
    } else {
      return [{login: 'Bob'}, {login: "Jo"}, {login: 'Avrel'}, {login: "Tao"}, {login: 'Kenzi'}, {login: "Jacques"}].sort(
        (a, b) => Math.round( 2*Math.random() - 1 )
      );
    }
  }
